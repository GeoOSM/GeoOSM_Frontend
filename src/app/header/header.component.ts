import { ViewChildren, Component, OnInit, Input, HostListener, Directive, NgIterable, Host, TemplateRef, ViewContainerRef, EmbeddedViewRef } from '@angular/core';
...
export class HeaderComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.communicationComponent.changeExtent.subscribe((value)=>{
            this.my_extend_active_form_group.get('my_extend_active').setValue(value,{emitEvent:false})
        })

        this.my_extend_active_form_group.get('my_extend_active').valueChanges.subscribe((value)=>{
            console.log(this.my_extend_active_form_group.get('my_extend_active').value)
            this.communicationComponent.changeExtent.next(this.my_extend_active_form_group.get('my_extend_active').value)
        })
        ...
        this.communicationComponent.getDataThematiques().subscribe(data => {
            ...
            for (var i = 0; i < donne['thematiques'].length; i++) {
                if (donne['thematiques'][i].sous_thematiques) {
                    for (var j = 0; j < donne['thematiques'][i].sous_thematiques.length; j++) {
                        for (var k = 0; k < donne['thematiques'][i].sous_thematiques[j].couches.length; k++) {


                            donne['thematiques'][i].sous_thematiques[j].couches[k]['tags_'] = ''
                            donne['thematiques'][i].sous_thematiques[j].couches[k]['rang_thema'] = i
                            var element = []

                            if (!Array.isArray(donne['thematiques'][i].sous_thematiques[j].couches[k]['metadata'])) {



                                for (var index = 0; index < donne['thematiques'][i].sous_thematiques[j].couches[k]['metadata'].tags.length; index++) {
                                    element.push(donne['thematiques'][i].sous_thematiques[j].couches[k]['metadata'].tags[index].tags);
                                }

                                donne['thematiques'][i].sous_thematiques[j].couches[k]['tags_'] = element.toString()

                            }

                            donne['thematiques'][i].sous_thematiques[j].couches[k]['description_catalogue'] = donne['thematiques'][i].sous_thematiques[j].couches[k]['nom'] + ' ' + donne['thematiques'][i]['nom'] + ' ' + donne['thematiques'][i].sous_thematiques[j]['nom'] + ' ' + element.toString()

                            this.reponse_geocode["thematiques"].push(donne['thematiques'][i].sous_thematiques[j].couches[k])
                    ...
                    for (var j = 0; j < donne['thematiques'][i].couches.length; j++) {
                        ...
                        var element = []
                        if (!Array.isArray(donne['thematiques'][i].couches[j]['metadata'])) {


                            for (var index = 0; index < donne['thematiques'][i].couches[j]['metadata'].tags.length; index++) {
                                element.push(donne['thematiques'][i].couches[j]['metadata'].tags[index].tags);
                            }

                            donne['thematiques'][i].couches[j]['tags_'] = element.toString()

                        }
                        donne['thematiques'][i].couches[j]['description_catalogue'] = donne['thematiques'][i].couches[j]['nom'] + ' ' + donne['thematiques'][i]['nom'] + ' ' + element.toString()

                        this.reponse_geocode["thematiques"].push(donne['thematiques'][i].couches[j])
            ...
            for (var i = 0; i < donne['cartes'].length; i++) {
                if (donne['cartes'][i].sous_cartes) {
                    for (var j = 0; j < donne['cartes'][i].sous_cartes.length; j++) {
                        for (var k = 0; k < donne['cartes'][i].sous_cartes[j].couches.length; k++) {

                            if (donne['cartes'][i].sous_cartes[j].couches[k].type == 'pdf') {

                                for (var index = 0; index < donne['cartes'][i].sous_cartes[j].couches[k].cartes_pdf.length; index++) {
                                    donne['cartes'][i].sous_cartes[j].couches[k].cartes_pdf[index]['nom'] = donne['cartes'][i].sous_cartes[j].couches[k].cartes_pdf[index]['name']
                                    donne['cartes'][i].sous_cartes[j].couches[k].cartes_pdf[index]['tags_'] = ''
                                    donne['cartes'][i].sous_cartes[j].couches[k].cartes_pdf[index]['rang_thema'] = i
                                    var element = []
                                    if (!Array.isArray(donne['cartes'][i].sous_cartes[j].couches[k].cartes_pdf[index]['metadata'])) {



                                        for (var z = 0; z < donne['cartes'][i].sous_cartes[j].couches[k].cartes_pdf[index]['metadata'].tags.length; z++) {
                                            element.push(donne['cartes'][i].sous_cartes[j].couches[k].cartes_pdf[index]['metadata'].tags[z].tags);
                                        }

                                        donne['cartes'][i].sous_cartes[j].couches[k].cartes_pdf[index]['tags_'] = element.toString()

                                    }

                                    donne['cartes'][i].sous_cartes[j].couches[k].cartes_pdf[index]['description_catalogue'] = donne['cartes'][i].sous_cartes[j].couches[k].cartes_pdf[index]['name'] + ' ' + donne['cartes'][i]['nom'] + ' ' + donne['cartes'][i].sous_cartes[j]['nom'] + ' ' + donne['cartes'][i].sous_cartes[j].couches[k]['nom'] + ' ' + element.toString()

                                    this.reponse_geocode["cartes"].push(donne['cartes'][i].sous_cartes[j].couches[k].cartes_pdf[index])
                                }
                                ...
                                var element = []
                                if (!Array.isArray(donne['cartes'][i].sous_cartes[j].couches[k]['metadata'])) {

                                    for (var index = 0; index < donne['cartes'][i].sous_cartes[j].couches[k]['metadata'].tags.length; index++) {
                                        element.push(donne['cartes'][i].sous_cartes[j].couches[k]['metadata'].tags[index].tags);
                                    }

                                    donne['cartes'][i].sous_cartes[j].couches[k]['tags_'] = element.toString()

                                }
                                donne['cartes'][i].sous_cartes[j].couches[k]['description_catalogue'] = donne['cartes'][i].sous_cartes[j].couches[k]['nom'] + ' ' + donne['cartes'][i]['nom'] + ' ' + donne['cartes'][i].sous_cartes[j]['nom'] + ' ' + element.toString()
                                this.reponse_geocode["cartes"].push(donne['cartes'][i].sous_cartes[j].couches[k])
                            }
                    ...
                    for (var k = 0; k < donne['cartes'][i].couches.length; k++) {
                        ...
                        if (donne['cartes'][i].couches[k].type == 'pdf') {
                            ...
                            for (var index = 0; index < donne['cartes'][i].couches[k].cartes_pdf.length; index++) {
                                donne['cartes'][i].couches[k].cartes_pdf[index]['description_catalogue'] = donne['cartes'][i].couches[k].cartes_pdf[index]['name'] + ' ' + donne['cartes'][i]['nom'] + ' ' + donne['cartes'][i].couches[k]['nom']
                                donne['cartes'][i].couches[k].cartes_pdf[index]['nom'] = donne['cartes'][i].couches[k].cartes_pdf[index]['name']
                                donne['cartes'][i].couches[k].cartes_pdf[index]['tags_'] = ''
                                donne['cartes'][i].couches[k].cartes_pdf[index]['rang_thema'] = i
                                this.reponse_geocode["cartes"].push(donne['cartes'][i].couches[k].cartes_pdf[index])
                            }

                        } else {
                            donne['cartes'][i].couches[k]['description_catalogue'] = donne['cartes'][i].couches[k]['nom'] + ' ' + donne['cartes'][i]['nom']

                            donne['cartes'][i].couches[k]['rang_thema'] = i
                            donne['cartes'][i].couches[k]['tags_'] = ''

                            this.reponse_geocode["cartes"].push(donne['cartes'][i].couches[k])
                        }
    ...
    geocode(word) {
        ...
        if (word.split(',').length == 1) {
            ...
            word_for_search_limite = word.replace('commune', '')
                .replace('communes', '')
                .replace('quartier', '')
                .replace('quartiers', '')
                .replace('departement', '')
    ...
    displayResult(item, type) {
        ...
        if (type == 'nominatim') {
            ...
            this.word_geocode = ''
            if (type == 'limites') {
                this.word_geocode = item.type_display + ' : ' + item.name + ' (' + item.ref + ')'
            }
            if (type == 'position') {
...
export class NgForIfEmpty<T> {
    ...
    constructor(@Host() private ngForOf: NgForOf<T>,
        ...
        ngForOf.ngDoCheck = () => {
            ...
            //   console.log(ngForOf)
            this.isEmpty = !ngForOf['_ngForOf'] || this.isIterableEmpty(ngForOf['_ngForOf']);
