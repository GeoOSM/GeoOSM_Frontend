...
export class DownloadsComponent implements OnInit {
    ...
    try {
      for (let index = 0; index < this.layerInMap.length; index++) {
        ...
        if (element.type_couche_inf == "thematiques") {
          var a = {
            couche_analyse: this.communicationComponent.get_couche_by_key_and_id_cat(
              element["id_cat"],
              element["key_couche"]
            ),
            thematiques_analyse: this.communicationComponent.get_thematique_by_rang(
              element.rang_thema
            ),
          };
  ...
  chooseThematique_function(i) {
    this.analyse_spatial["thematiques_analyses"][i][
      "couche_analyse"
    ] = undefined;
  }
    ...
    if (!this.autocomplteActive) {
      this.myControl.valueChanges
        .pipe(
          ...
          tap((value) => {
            this.recherche_is_loading = true;
            this.analyse_spatial["emprises"] = [];
            this.analyse_spatial[
              this.analyse_spatial["type_emprise_spatiale"]
            ] = [];
          }),
          switchMap((value) => {
            if (typeof value === "string" && value.length > 1) {
              return this.geoportailService
                .searchLimiteObservable({
                  // 'table':this.paramsForqueryAutocomplete.table,
                  word: value.toString(),
                })
                .pipe(finalize(() => (this.recherche_is_loading = false)));
            }
          })
        )
  ...
  chooseCouche_function() {
    ...
    if (
      ...
      this.analyse_spatial["type_emprise_spatiale"] != "draw"
    ) {
      this.paramsForqueryAutocomplete = {
        table: this.analyse_spatial["type_emprise_spatiale"],
      };
  ...
  disableOptionAnalyseSpatial(data) {
    if (data.type == "xyz") {
      ...
      if (
        data.cles_vals_osm != undefined &&
        (data.cles_vals_osm.length > 0 ||
          (data.categorie && data.categorie.mode_sql)) > 0 &&
        !this.disabled_couche(data)
      ) {
        return false;
      } else {
        return true;
  ...
  display_calculate_result_analyse_spatial() {
    if (
      this.analyse_spatial["type_emprise_spatiale"] &&
      this.analyse_spatial["emprises"] &&
      this.analyse_spatial["emprisesChoisiId"]
    ) {
      var error = [];
      for (
        var index = 0;
        index < this.analyse_spatial["thematiques_analyses"].length;
        index++
      ) {
        if (
          this.analyse_spatial["thematiques_analyses"][index][
            "thematiques_analyse"
          ] &&
          this.analyse_spatial["thematiques_analyses"][index]["couche_analyse"]
        ) {
        } else {
          error.push(1);
        }
  ...
  calculate_result_analyse_spatial() {
    ...
    for (
      ...
      var couche = this.analyse_spatial["thematiques_analyses"][index][
        "couche_analyse"
      ];
      var thematique = this.analyse_spatial["thematiques_analyses"][index][
        "thematiques_analyse"
      ];
    ...
    if (
      ...
      this.geoportailService
        .getLimitById({
          table: this.analyse_spatial["table"],
          id: this.analyse_spatial["emprisesChoisiId"],
        })
        .then((data: Object[]) => {
      ...
      for (
        let index = 0;
        index < this.analyse_spatial["query"].length;
        index++
      ) {
        const element = this.analyse_spatial["query"][index];

        var couche = this.communicationComponent.get_couche_by_key_and_id_cat(
          element["id_them"],
          element["key_couche"]
        );
        ...
        var nom_shp =
          ...
          "/" +
          environment.pojet_nodejs +
          "/gpkg/" +
          params_files.nom_cat.replace(/[^a-zA-Z0-9]/g, "_") +
          "_" +
          params_files.sous_thematiques +
          "_" +
          params_files.key_couche +
          "_" +
          params_files.id_cat +
          ".gpkg";
